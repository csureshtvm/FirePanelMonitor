jQuery(function($) {
  ...
  var ApiSelectionController = Spine.Controller.create({
    ...
    init: function() {
      if (this.supportsLocalStorage()) {
        var baseUrl = localStorage.getItem("ui.baseUrl");
        var apiKey = localStorage.getItem("ui.apiKey");
        var secretKey = localStorage.getItem("ui.secretKey");
        var format = localStorage.getItem("ui.format");
        
	      if (baseUrl && baseUrl.length > 0)
	      $("#input_baseUrl").val(baseUrl);
	      if (apiKey && apiKey.length > 0)
	      $("#input_apiKey").val(this.apiKey);
	      if (secretKey && secretKey.length > 0)
	      $("#input_secretKey").val(this.secretKey);
	      if (format && format.length > 0)
	      $("#input_format").val(this.format);
    ...
    handleEnter: function(){
      ...
      $('#input_secretKey').keydown(function(e) {
          if(e.which != 13) return;
          submit();
        });
      $('#input_format').keydown(function(e) {
          if(e.which != 13) return;
          submit();
        });      
    },
    ...
    showApi: function() {
      ...
      var secretKey = jQuery.trim($("#input_secretKey").val());
      var format = jQuery.trim($("#input_format").val());
      if (baseUrl.length == 0) {
        $("#input_baseUrl").wiggle();
      } else {
        if (this.supportsLocalStorage()) {
          localStorage.setItem("ui.apiKey", apiKey);
          localStorage.setItem("ui.baseUrl", baseUrl);
          localStorage.setItem("ui.secretKey", secretKey);
          localStorage.setItem("ui.format", format);
        }
  ...
  var ResourceListController = Spine.Controller.create({
    ...
    init: function() {
      ...
      var swaggerService = new SwaggerService(this.baseUrl, this.apiKey, this.secretKey, this.format,
      function(msg) {
        if (msg)
