...
public class SoftwareAlarmsTabUI extends JPanel{
	JTable alarmEventsTable;
	Logger log = Logger.getLogger("SoftwareAlarmsTabUI");
	//static Color[] rowColors={Color.red,Color.cyan,Color.cyan};
	DefaultTableModel model =null;
	 JScrollPane jsp=null;
	
	 public List<SoftwareMessageDTO> alarmEventList=new ArrayList<SoftwareMessageDTO>();
	 
	 public static List<Integer> alarms=new ArrayList<Integer>();
	 //ButtonColumn buttonColumn=null;
	 int totalTableWidth=500;
	public SoftwareAlarmsTabUI(){
		String[] columns = new String[] {
	            "Sl #", "Building", "Event Name", "Status Value","Event Time"
	        };
        ...
        for(int i=0;i<alarmEventsTable.getColumnCount();i++){
        	...
        	column.setCellRenderer(new CustomSWCellRenderer());
	...
	private void setScreenPosition(){
		...
		int[] colSizeArr=new int[]{5,35,20,20,20};
		 for(int i=0;i<alarmEventsTable.getColumnCount();i++){
	        	TableColumn column=alarmEventsTable.getColumnModel().getColumn(i);
	...
	public void populateData(List<SoftwareMessageDTO> fireAlarmEventList, List<Integer> fireAlarms){
		...
		int i=0;
		for(SoftwareMessageDTO  eventDTO : alarmEventList){
			
			Object[] data=new String[]{String.valueOf((++i)),eventDTO.getBuildingName(),eventDTO.getEventDetails(),eventDTO.getEventValue(),eventDTO.getEventUpdatedTime()!=null?String.valueOf(eventDTO.getEventUpdatedTime()):""};
			model.addRow(data);
		}
...
class CustomSWCellRenderer extends DefaultTableCellRenderer {
	Border border=new LineBorder(Color.DARK_GRAY);
    @Override
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
        JLabel l = (JLabel) super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
        //if (hasFocus) {
        if(SoftwareAlarmsTabUI.alarms.contains(row)){
