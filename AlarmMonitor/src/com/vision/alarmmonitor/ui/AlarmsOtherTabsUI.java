...
public class AlarmsOtherTabsUI extends JPanel{
	...
	public AlarmsOtherTabsUI(){
		String[] columns = new String[] {
	            "Sl #","Building", "System Name" ,"Signal", "Status","Date Time", ""
	        };
        ...
        for(int i=0;i<alarmEventsTable.getColumnCount();i++){
        	...
        	column.setCellRenderer(new CustomCellRendererOtherAlarms());
        ...
        buttonColumn = new ButtonColumn(alarmEventsTable, reset, 6,"Reset","other");
	...
	private void setScreenPosition(){
		...
		int[] colSizeArr=new int[]{5,30,10,15,10,20,10};
		 for(int i=0;i<alarmEventsTable.getColumnCount();i++){
	        	TableColumn column=alarmEventsTable.getColumnModel().getColumn(i);
	...
	public void populateData(List<AlarmEventDTO> fireAlarmEventList, List<Integer> otherAlarms){
		alarmEventList.clear();
		alarms.clear();
		
		alarmEventList.addAll(fireAlarmEventList);
		alarms.addAll(otherAlarms);
		...
		for(AlarmEventDTO  eventDTO : alarmEventList){
			...
			Object[] data=new String[]{String.valueOf((++i)),eventDTO.getBuildingName(),eventDTO.getEventSystem(),eventDTO.getEventSignal(),eventDTO.getEventValue(),eventDTO.getEventGeneratedTime()!=null?String.valueOf(eventDTO.getEventGeneratedTime()):"","Reset"};
			model.addRow(data);
		}
	    ...
	    buttonColumn.setMnemonic(KeyEvent.VK_D);
		buttonColumn.initializeTableModel(alarmEventsTable,6);
		// alarmEventsTable.setShowHorizontalLines(true);
 		//alarmEventsTable.setShowVerticalLines(true);
...
class CustomCellRendererOtherAlarms extends DefaultTableCellRenderer {
	Border border=new LineBorder(Color.DARK_GRAY);
    @Override
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
        JLabel l = (JLabel) super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
        //if (hasFocus) {
        if(AlarmsOtherTabsUI.alarms.contains(row)){
